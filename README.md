...
without detecting it, `?raw` serves a file's source or a root's raw bytes,
`?download` saves a file under its collection name, and `?sizes` adds file
sizes to a listing. The same content is served on
